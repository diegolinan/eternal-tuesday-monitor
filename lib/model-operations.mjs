...
export function compileModelOperations({
  ...
  return {
    ...
    models: monitorModels.map((model) => {
      ...
      const catalogOnly = model.relevanceState === 'CATALOG_ONLY';
      ...
      return {
        ...
        eligibilityCheck: {
          state: catalogOnly
            ? 'NOT_IN_SCOPE'
            : (live?.eligibility?.state ?? model.testabilityState),
          checkedAt: catalogOnly ? null : live ? generatedAt : null,
          stateChangedOn: catalogOnly
            ? (model.relevanceReviewedOn ?? null)
