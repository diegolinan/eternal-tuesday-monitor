...
const API =
  'https://api.github.com/repos/diegolinan/eternal-tuesday-monitor/actions/workflows/discover-models.yml/runs?per_page=20';
...
export function AutomationStatus() {
  ...
  const next = useMemo(() => (now ? nextScheduled(now) : null), [now]);
  const stale =
    latest && now
      ? now.valueOf() - new Date(latest.created_at).valueOf() > 36 * 3600 * 1000
      : false;
    ...
    >
      ...
      <dl>
        <div>
          <dt>Last attempt</dt>
          <dd>
            {loaded ? displayDate(latest?.created_at ?? null) : 'READING…'}
            {latest && (
              <small>
                {latest.status.toUpperCase()} ·{' '}
                {(latest.conclusion ?? 'PENDING').toUpperCase()}
              </small>
            )}
          </dd>
        </div>
        <div>
          <dt>Last success</dt>
          <dd>
            {loaded ? displayDate(success?.updated_at ?? null) : 'READING…'}
          </dd>
        </div>
        <div>
          <dt>Next scheduled attempt</dt>
          <dd>
            {next ? displayDate(next) : 'CALCULATING…'}
            {next && now && (
              <small>ESTIMATED · IN {remaining(next, now)}</small>
            )}
          </dd>
        </div>
      </dl>
      {stale && (
        <p className="automation-warning">
          No discovery attempt is visible in the last 36 hours. The public
          status feed may be delayed.
        </p>
      )}
    </section>
  );
