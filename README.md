...
The [review-gated automated candidate flow](docs/automated-probe-candidates.md) is deliberately narrower than general probe execution: one OpenAI model, one temporal-anchor protocol, three invocations, and at most one run per day. Its weekly/manual workflow can only write a sanitized, non-canonical candidate and open a draft PR. It cannot merge, accept evidence, append an observation, compile a new public result, or publish the Site. Paid execution is currently disabled in `config/automated-probe-candidate.json`; scheduled runs therefore stop before reading the key or making a network request. Enabling it later requires an explicit reviewed policy change and active OpenAI API billing.

Optional discovery credentials are OPENAI_API_KEY, ANTHROPIC_API_KEY, GEMINI_API_KEY and XAI_API_KEY in GitHub Secrets. Missing credentials skip authenticated listing, without disabling public discovery. GET requests have timeout, response-size, redirect, pagination and detail-count limits. HTTPS host allowlists are checked at every redirect; authenticated redirects cannot cross origins. Credentials are never placed in URLs, source files, PR text or artifacts. Discovery never calls a generative endpoint; the separately approved manual pilot uses a local key, not a scheduled workflow.

A failed source creates an adapter outcome, not catalog deletions. Authenticated errors can propose an account-access ERROR/ACCESS_DENIED snapshot while retaining prior identity and availability provenance. Fresh account confirmations are cached for at most seven days to avoid daily renewal-only PRs; failure checks invalidate readiness. The manual pilot checks account availability, binds approval to exact code/parameters, limits calls and output tokens, reserves estimated spend, and respects its own kill switch. The automated candidate harness is fixture-tested but its paid execution gate remains disabled; automatic evidence acceptance is not implemented.
...
Daily model discovery and display-freshness refresh are configured as described above. A separately approved manual OpenAI pilot can save private evidence candidates. The bounded weekly OpenAI candidate workflow is installed but exits before network access while its execution policy is disabled. Automated evidence acceptance is not configured.
